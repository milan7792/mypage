...
public class ArticleFileController {
    ...
    public ResponseEntity<List<String>> fileList(@PathVariable("article_no") Integer article_no) throws Exception {
        ...
        try {
            entity = new ResponseEntity<List<String>>(articleFileService.getArticleFiles(article_no), HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
 ...
 // 업로드 파일 보여주기
    ...
    public ResponseEntity<String> boardWriteRemoveFile(String fileName) throws Exception {
	  ...
	  // 단일 파일 데이터 삭제2 : 게시글 수정화면
		 ...
		 * boardMofifyRemoveFile(@PathVariable("article_no") Integer article_no, String
		 * fileName, HttpServletRequest request) throws Exception { // DB에서 파일명 제거
		 * articleFileService.deleteAttach(fileName); // 첨부파일 갯수 갱신
		 * articleFileService.updateAttach(article_no); // 파일 삭제
		 * UploadFileUtils.removeFile(uploadPath, fileName); return new
